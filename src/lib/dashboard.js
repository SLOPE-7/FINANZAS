import { listAccounts, esLiquida } from './accounts.js'
...
export async function loadDashboard() {
  ...
  // Disponible = solo lo que está a mano hoy.
  const saldo = cuentas
    .filter(c => esLiquida(c.type))
    .reduce((s, c) => s + Number(c.balance), 0)

  const apartado = cuentas
    .filter(c => c.type === 'ahorro')
    .reduce((s, c) => s + Number(c.balance), 0)

  const porCobrar = cuentas
    .filter(c => c.type === 'por_cobrar')
    .reduce((s, c) => s + Number(c.balance), 0)
